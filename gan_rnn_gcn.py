...
class GAN_RNN_GCN():
    def __init__(self, g_input_step=14, g_input_size=28, g_hidden_size=50, g_output_step=28, g_batch_size=50, g_rate=2e-4, \
                 ...
                 num_epochs=100, print_interval=10, num_support=5, graph_file="graph.txt", data_file="diffusion.pkl"):
    ...
    def generator(self, input, input_step, input_size, hidden_size, batch_size, reuse=False):
        with tf.variable_scope("generator") as scope:
            ...
            g_W_o = tf.Variable(tf.random_normal([hidden_size, input_size]))
            ...
            g_y_ = tf.reshape(g_y_soft, [batch_size, input_step, input_size])

            # concentrate input and output of rnn
            x = tf.concat([input, g_y_], axis=1)
            return x
    ...
    def discriminator(self, input, input_step, input_size, hidden_size, output_size, batch_size, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            d_W_o = tf.Variable(tf.random_normal([hidden_size / 2, output_size]))
            ...
            d_output = tf.reshape(tf.concat(d_outputs, axis=1), [-1, input_step, hidden_size / 2])

            d_y = tf.matmul(d_output[:, -1], d_W_o) + d_b_o  # d_y, [batch_size, 1]
            return d_y
